        ...
        match index {
            FROM_USER => match from_user.try_recv() {
                Ok(UserInput::CloseCommand) => {
                    connection.close(quad).await.unwrap();
                }
                Ok(UserInput::SendData(data)) => {
                    connection.send(quad, data.to_vec()).await.unwrap();
                }
                Err(_) => { /* TODO: decide */ }
            },
            FROM_SERVER => match from_server.try_recv() {
                Ok(buffer) => {
                ...
                Err(e) => {
                    tracing::error!(
                        error = %e,
                        "Failed to receive message from the server channel"
                    )
                }
            },
            _ => {}
...
pub async fn run(quad: Quad, nic: Arc<dyn AsyncTun + Sync + Send>) {
    ...
    connection.open(quad).await.unwrap();
